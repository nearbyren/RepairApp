...
abstract class BaseLazyLoadFragment : Fragment(), ViewBehavior, NetworkListenerHelper.NetworkConnectedListener {
    ...
    protected fun initViewHeader(view: View?) {
        //标题栏适配器
        val barHelper = ToolBarHelperUtil.builder()
        //标题栏config
        val barHelperConfig = initBarHelperConfig()
        barHelperConfig?.activity = requireActivity() as BaseActivity
    ...
    private fun initState(view: View?) {
        ...
        if (res > 0) {
            view?.let {
                stateManager = StateBox.default.inject(it, res, object : StateBox.OnReloadListener {
